...
function powershellArgs(script: string, params: string[]): string[] {
...
export class WindowsDriver {
  ...
  private visionEnv(): Record<string, string> {
    const env: Record<string, string> = {}
    if (this.config.visionApiKey) env.VISION_API_KEY = this.config.visionApiKey
    if (this.config.visionModel) env.VISION_MODEL = this.config.visionModel
    if (this.config.visionBaseUrl) env.VISION_BASE_URL = this.config.visionBaseUrl
    return env
  }
   ...
   */
  async see(prompt: string, windowTitle?: string): Promise<string> {
    const args = ['-Prompt', prompt]
    if (windowTitle) args.push('-WindowTitle', windowTitle)
    ...
    if (r.exitCode !== 0) throw new Error(`see.ps1 failed: ${r.stderr || r.stdout}`)
   ...
   */
  async capture(outFile: string, windowTitle?: string): Promise<CaptureMeta> {
    const args = ['-Out', outFile]
    if (windowTitle) args.push('-WindowTitle', windowTitle)
    const r = await this.run('powershell', powershellArgs(scriptPath('capture.ps1'), args), {
      timeoutMs: this.config.commandTimeoutMs,
    })
    if (r.exitCode !== 0) throw new Error(`capture.ps1 failed: ${r.stderr || r.stdout}`)
   ...
   */
  async vision(imagePath: string, prompt: string): Promise<VisionResult> {
    const r = await this.run('node', [scriptPath('vision.js'), imagePath, prompt], {
      timeoutMs: this.config.visionTimeoutMs,
      env: this.visionEnv(),
    })
    if (r.exitCode !== 0) throw new Error(`vision.js failed: ${r.stderr || r.stdout}`)
  ...
  async click(x: number, y: number): Promise<void> {
    const r = await this.run('powershell',
      powershellArgs(scriptPath('input.ps1'), ['click', '-X', String(x), '-Y', String(y)]),
      { timeoutMs: this.config.commandTimeoutMs })
    if (r.exitCode !== 0) throw new Error(`click failed: ${r.stderr || r.stdout}`)
  }

  /** Simulate a right-click at screen coordinates. */
  async rightclick(x: number, y: number): Promise<void> {
    const r = await this.run('powershell',
      powershellArgs(scriptPath('input.ps1'), ['rightclick', '-X', String(x), '-Y', String(y)]),
      { timeoutMs: this.config.commandTimeoutMs })
    if (r.exitCode !== 0) throw new Error(`rightclick failed: ${r.stderr || r.stdout}`)
  }

  /** Simulate a mouse drag from one point to another. */
  async drag(fromX: number, fromY: number, toX: number, toY: number, modifiers?: string): Promise<void> {
    ...
    if (r.exitCode !== 0) throw new Error(`drag failed: ${r.stderr || r.stdout}`)
  }

  /** Type text via clipboard paste (recommended for CJK). */
  async type(text: string): Promise<void> {
    const r = await this.run('powershell',
      powershellArgs(scriptPath('input.ps1'), ['type', '-Text', text]),
      { timeoutMs: this.config.commandTimeoutMs })
    if (r.exitCode !== 0) throw new Error(`type failed: ${r.stderr || r.stdout}`)
  }

  /** Send a keyboard shortcut or key combination. */
  async key(keys: string): Promise<void> {
    const r = await this.run('powershell',
      powershellArgs(scriptPath('input.ps1'), ['key', '-Keys', keys]),
      { timeoutMs: this.config.commandTimeoutMs })
    if (r.exitCode !== 0) throw new Error(`key failed: ${r.stderr || r.stdout}`)
  }

  /** Scroll the mouse wheel at screen coordinates. */
  async scroll(x: number, y: number, delta: number): Promise<void> {
    const r = await this.run('powershell',
      powershellArgs(scriptPath('input.ps1'), ['scroll', '-X', String(x), '-Y', String(y), '-Delta', String(delta)]),
      { timeoutMs: this.config.commandTimeoutMs })
  ...
  async knowledge(action: string, query?: string): Promise<string> {
    const args: string[] = []
    if (action === 'list') args.push('-List')
    else if (action === 'search' && query) args.push('-Search', query)
    else if (action === 'app' && query) args.push('-App', query)
    else throw new Error(`unknown knowledge action: ${action}`)
    const r = await this.run('powershell',
      powershellArgs(scriptPath('knowledge.ps1'), args),
      { timeoutMs: this.config.commandTimeoutMs })
    if (r.exitCode !== 0) throw new Error(`knowledge.ps1 failed: ${r.stderr || r.stdout}`)
    return r.stdout.trim()
  }
}
